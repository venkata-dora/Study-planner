export default function LearningLessonHeader({ section, done, total, unit = 'topics', onRead, children }) {
  ...
  return <header className="study-lesson-header">
    <div className="apple-page-heading"><div><span className="learning-eyebrow">{section.subsections.length} {section.subsections.length === 1 ? 'SECTION' : 'SECTIONS'} · {total} {unit.toUpperCase()}</span><h1>{section.title.replace(/^\d+\s*·\s*/, '')}</h1></div>{onRead && <button className="btn btn-secondary" onClick={onRead}>Read topic guide</button>}</div>
