...
export default function LearningRoadmap({ data, title, description, route, unit = 'topics', children }) {
  ...
  const journey = <JourneyMap title={title} stages={sections.map(s => ({ ...s, items: s.subsections.map((sub, i) => ({ id: `${s.id}-${i}`, title: sub.label.replace(/^[★◆○]\s*/, ''), to: `${route}/${s.id}` })) }))} />
  ...
  return <div className="learning-overview">
    ...
    {journey}<details className="journey-all"><summary>Browse all course contents</summary><div className="learning-section-title"><h2>Course contents</h2><span>{sections.length} sections</span></div>
