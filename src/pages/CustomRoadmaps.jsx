...
function CustomPath({ id }) {
  ...
  return <div className="learning-overview roadmap-studio">
    ...
    {error && <p role="alert">{error}</p>}
    <JourneyMap title={map.title} stages={map.stages.map(s => ({ id: s.id, title: s.title, description: s.outcome, total: s.topics.length, done: s.topics.filter(t => map.lessons[t.id]?.completed).length, items: s.topics.map(t => ({ id: t.id, title: t.title, completed: map.lessons[t.id]?.completed, onSelect: () => choose(t) })) }))} />
    <div className="path-workspace"><details className="journey-all"><summary>Browse all stages and lessons</summary><div className="path-timeline">{map.stages.map((s, i) => <section className="path-stage" key={s.id}><span className="path-marker">{String(i + 1).padStart(2, '0')}</span><div className="path-stage-content"><span className="learning-eyebrow">STAGE {i + 1}</span><h2>{s.title}</h2><p>{s.outcome}</p><div className="path-topics">{s.topics.map(t => <button className={`path-topic${selected?.id === t.id ? ' selected' : ''}`} aria-pressed={selected?.id === t.id} key={t.id} onClick={() => choose(t)}><span>{map.lessons[t.id]?.completed ? '✓' : '○'}</span><span>{t.title}<small>{t.subtopics.length} subtopics</small></span><span aria-hidden="true">↗</span></button>)}</div></div></section>)}</div></details>
