...
export default function ReadingWorkspace() {
  ...
  return <div className="reading-workspace">
    <div className="reader-navigation"><button className="reader-sidebar-control" aria-label={open ? 'Collapse learning path' : 'Expand learning path'} aria-expanded={open} aria-controls="reader-path" title={open ? 'Collapse learning path' : 'Expand learning path'} onClick={() => setOpen(value => !value)}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><path d={open ? 'M14 6l-6 6 6 6' : 'M10 6l6 6-6 6'} /></svg></button><Link to={location.state?.returnTo || course?.route || '/blogs'}>← {course?.title || 'Reading library'}</Link></div>
