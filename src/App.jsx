...
export default function App() {
  ...
  return <div className={`learning-platform apple-platform${menuOpen ? ' menu-open' : ''}${editorPage ? ' editor-page' : ''}${readerPage ? ' reader-mode' : ''}`}>
    ...
    <aside ref={sidebarRef} id="study-navigation" inert={readerPage && !menuOpen ? true : undefined} className="apple-sidebar" role={menuOpen ? 'dialog' : undefined} aria-modal={menuOpen || undefined} aria-label="Learning navigation">
      <button className="apple-menu-close btn btn-secondary" onClick={closeMenu} aria-label="Close navigation">{readerPage ? <LearningIcon name="menu" /> : 'Close menu'}</button>
